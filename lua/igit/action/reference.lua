...
local common = require("igit.action.common")
...
function M.reset(current, target)
    ...
    mode = mode:split(":")[1]
    if mode then
        common.runasync_and_reload(git.reset(mode, target))
    end
end

function M.confirm_rebase(opts)
    if not opts.confirm_rebase then
        return true
    end
...
function M.rebase_branches(opts)
    vim.validate({
        ...
        confirm_rebase = { opts.confirm_rebase, "b", true },
    })

    if not M.confirm_rebase(opts) then
        return
    end
