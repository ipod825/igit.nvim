...
function M:add_argument(provided_name, opts)
    ...
    assert(provided_name ~= self.commands_key,
...
function M:parse(str, return_hierarchical_result)
    ...
    local res, err_msg = self:parse_internal(str:split(),
...
function M:get_completion_list_internal(args, hint)
    ...
    for k, arg_prop in pairs(flag_arg_props) do
        if parsed_res[k] == nil then res:append(arg_prop.repr) end
    end

    if hint then
        res = res:filter(function(e) return e:startswith(hint) end):collect()
    end
    table.sort(res)
    return res
end
...
function M:parse_internal(args, return_hierarchical_result)
    ...
    if return_hierarchical_result then return {{self.prog, res}} end
    return res
end
