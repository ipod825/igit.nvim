...
function M:refresh()
    if self.root ~= nil then
        job.run(('rm -rf %s'):format(self.root))
        job.run(('cp -r %s_bak %s'):format(self.root, self.root))
        return self.root
    end
    self.root = self:create_dir()
    job.run(('cp -r %s %s_bak'):format(self.root, self.root))
    return self.root
end

function M:remove_dir(d) job.run(('rm -rf %s %s_bak'):format(d, d)) end

function M:current_branch() return job.popen(self.git.branch('--show-current')) end

function M:create_dir()
    local root = '/tmp/igit-test'
    job.run(('rm -rf %s %s_bak'):format(root, root))
    assert(uv.fs_mkdir(root, 448), "Faile to create directory")
    local run = function(cmd) job.run(cmd, {cwd = root}) end
