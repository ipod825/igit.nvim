...
function M:setup(options)
    ...
    self.options = vim.tbl_deep_extend("force", {
        mappings = {
            n = {
                ["H"] = { self:BIND(self.stage_change), desc = "Stage (`git add`) the current file" },
                ["L"] = { self:BIND(self.unstage_change), desc = "Unstage (`git restore --staged`) the current file" },
                ["X"] = { self:BIND(self.discard_change), desc = "Discard (`git restore`) the current file" },
                ...
                ["t"] = { self:BIND(self.open_file, "tab drop"), desc = "Open the current file in a new tab" },
            },
            v = {
                ["H"] = { self:BIND(self.stage_change), desc = "Stage (`git add`) the visually selected files" },
                ["L"] = {
                    self:BIND(self.unstage_change),
                    desc = "Unstage (`git restore --staged`) visually selected files",
                },
                ["X"] = { self:BIND(self.discard_change), desc = "Discard (`git restore`) visually selected files" },
                ["C"] = {
                    self:BIND(self.clean_files),
