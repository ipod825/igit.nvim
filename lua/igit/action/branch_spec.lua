    ...
    describe("functions", function()
        ...
        describe("parse_line", function()
            a.it("Parses the information of the lines", function()
                ...
                assert.are.same({ branch = "branch-name", is_current = true }, action.branch.parse_line())
                util.set_current_line(" branch-name Commit message (with paranthesis)")
                assert.are.same({ branch = "branch-name", is_current = false }, action.branch.parse_line())
                util.set_current_line("* (HEAD detached at a27b03661) a27b03661 Commit message (with paranthesis)")
                assert.are.same({ branch = "HEAD", is_current = true }, action.branch.parse_line())
            end)
        end)
            ...
            a.it("Switches the branch", function()
                local function set_line_with_different_branch()
                    ...
                    for i = 1, vim.fn.line("$") do
                        local branch = action.branch.parse_line(i).branch
        ...
        describe("rename", function()
            a.it("Renames branches", function()
                ...
                assert.are.same({
                    ...
                    is_current = true,
                }, action.branch.parse_line(1))
                assert.are.same({
                    branch = util.new_name(test_dir.path1[2]),
                    is_current = false,
                }, action.branch.parse_line(2))
            end)
        end)
